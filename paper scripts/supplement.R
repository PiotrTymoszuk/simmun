...
# Figure S1: CONSORT -----
  ...
  insert_msg('Figure S1: study inclusion diagram')

  suppl$inclusion <-
    plot_grid(ggdraw() +
                draw_image('./schemes/inclusion.png')) %>%
    as_figure(label = 'figure_s1_study_inclusion_scheme',
              ref_name = 'inclusion',
...
# Figure S2: consistency of psychometric tools ------

  insert_msg('Figure S2: consistency of psychometric tools')
  ...
  suppl$consistency <- cons$fa_loading_plots %>%
    ...
    plot_grid(plot_grid(cons$omega_plot +
              ...
              labels = LETTERS,
              label_size = 10) %>%
    as_figure(label = 'figure_s2_psycho_consistency',
...
# Figure S3: error and R-squared, multiple linear regression ------

  insert_msg('Figure S3: Fits stats, multiple regression')
  ...
  suppl$fit_stats <- mod_eli$fit_stat_plots %>%
    ...
    as_figure(label = 'figure_s3_fit_stats',
              ref_name = 'fit_stats',
...
# Figure S4: univariate analysis for TRP/KYN ------

  insert_msg('Figure S4: Univariate analysis for TRP/KYN')
  ...
  suppl$uni_trp <- plot_grid(suppl$uni_trp$upper,
                            ...
                            labels = LETTERS,
                            label_size = 10) %>%
    as_figure(label = 'figure_s4_univariate_analysis_trp',
              ref_name = 'uni_trp',
...
# Figure S5: univariate analysis for the TYR subsystem -----

  insert_msg('Figure S5: univariate analysis for TYR')
  ...
  suppl$uni_tyr <- plot_grid(suppl$uni_tyr$upper,
                            ...
                            labels = LETTERS,
                            label_size = 10) %>%
    as_figure(label = 'figure_s5_univariate_analysis_tyr',
              ref_name = 'uni_tyr',
...
# Figure S6: INCOV, correlograms -----

  insert_msg('Figure S6: correlograms')
  ...
  suppl$corr_incov <- cyt_corr$bubbles %>%
    ...
    as_figure(label = 'figure_s6_incov_correlations',
